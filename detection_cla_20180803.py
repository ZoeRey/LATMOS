...
plt.plot(data532paral[66][:260],altitude[:260])
plt.grid()
plt.show()
...
for i in range(len(time)):
    for j in range(len(altitude)):
       data532paral[i][j]=data532paral[i][j]/geom532_fit[j]
"""
...
for i in range(len(iymax)):
    if i<=3:
        data532_medianfilter[i]=data532_medianfilter[i]+list(data532paral[i+62][:40])
        xbis[i]=xbis[i]+list(altitude[:40])   
    elif i==4:
        data532_medianfilter[i]=data532_medianfilter[i]+list(data532paral[i+62][:50])
        xbis[i]=xbis[i]+list(altitude[:50]) 
    elif i>4 and i<=8:
        data532_medianfilter[i]=data532_medianfilter[i]+list(data532paral[i+62][:80])
        xbis[i]=xbis[i]+list(altitude[:80])   
    else:
        data532_medianfilter[i]=data532_medianfilter[i]+list(scipy.signal.medfilt(data532paral[i+62][iymax[i]-20:iymax[i]+120]))
        xbis[i]=xbis[i]+list(altitude[iymax[i]-20:iymax[i]+120])
...
plt.figure()
plt.plot(datafit[89-49],altitude[iymax[89-49]-6:iymax[89-49]+60],color='black',linewidth=2.5,linestyle='--',label="approximation polynômiale")
plt.plot(data532paral[89][:200],altitude[:200],linewidth=0.75,label="PR2 532//")
plt.xlabel("signal rétrodiffusé, 532// nm")
plt.ylabel("altitude (km)")
plt.legend()
plt.show()
...
plt.figure()
plt.plot(time_hcla[10:52],deriv_hcla[10:])
plt.title(dates[0])
plt.xlabel("time (UTC)")
