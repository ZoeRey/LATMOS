...
#%%
plt.figure()
plt.plot(data532paral[42+38][:300],altitude[:300])
plt.show()
...
for i in range(42,108): 
    y=list(data532paral[i][:102])
    iymax.append(y.index(max(y)))


#%%filtre médian
    
time_hcla=time[42:108]
data532_medianfilter=[[] for i in range(len(time_hcla))]
xbis=[[] for i in range(len(time_hcla))]

for i in range(len(iymax)):
    if i<25:
        data532_medianfilter[i]=data532_medianfilter[i]+list(data532paral[i+40][10:43])
        xbis[i]=xbis[i]+list(altitude[10:43])      
    elif i>=25 and i<=37:
        data532_medianfilter[i]=data532_medianfilter[i]+list(data532paral[i+40][10:70])
        xbis[i]=xbis[i]+list(altitude[10:70])    
    else:
        data532_medianfilter[i]=data532_medianfilter[i]+list(scipy.signal.medfilt(data532paral[i+40][iymax[i]-20:iymax[i]+80]))
        xbis[i]=xbis[i]+list(altitude[iymax[i]-20:iymax[i]+80])
...
hcla=[]
#poser la question !!!!
print("A 16h45h, le sommet de la couche limite était plutôt à :")
...
hcla2=[]
#poser la question !!!!
print("A 8h, le sommet de la couche limite était plutôt à :")
