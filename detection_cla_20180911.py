...
#%%
plt.figure()
plt.plot(data532paral[40][:300],altitude[:300])
plt.show()
...
for i in range(40,102): #de 8h à 15hde 40 à 102
    y=list(data532paral[i][:140])
    iymax.append(y.index(max(y)))


#%%filtre médian
    
time_hcla=time[40:102]
data532_medianfilter=[[] for i in range(len(time_hcla))]
xbis=[[] for i in range(len(time_hcla))]

for i in range(len(iymax)):
    if i<6:
        data532_medianfilter[i]=data532_medianfilter[i]+list(data532paral[i+40][17:43])
        xbis[i]=xbis[i]+list(altitude[17:43])      
    else:
        data532_medianfilter[i]=data532_medianfilter[i]+list(scipy.signal.medfilt(data532paral[i+40][iymax[i]-20:iymax[i]+120]))
