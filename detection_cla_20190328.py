...
plt.plot(data532paral[49+22][:125],altitude[:125])
plt.grid()
plt.show()
...
for i in range(49,102):
    y=list(data532paral[i][:125])
    iymax.append(y.index(max(y)))

#%%filtre médian
    
time_hcla=time[49:102]
data532_medianfilter=[[] for i in range(len(time_hcla))]
xbis=[[] for i in range(len(time_hcla))]

for i in range(0,23):
    data532_medianfilter[i]=data532_medianfilter[i]+list(scipy.signal.medfilt(data532paral[i+49][iymax[i]-10:80]))
    xbis[i]=xbis[i]+list(altitude[iymax[i]-10:80])   
...
pt_inflexion=[[] for i in range(0,23)]
...
hcla=[]
#poser la question !!!!
print("A "+str(time[49+23])+", le sommet de la couche limite était plutôt à :")
print(pt_inflexion[len(pt_inflexion)-1])
hcla_end=input("Choisissez: ")
hcla_end=int(hcla_end)
hcla.append(pt_inflexion[len(pt_inflexion)-1][hcla_end])
...
for i in range(23,102-49): #de 6h à 17h
    y=list(data532paral[i+49][:125])
    iymax.append(y.index(max(y)))
    hcla.append(altitude[iymax[i]])
...
hcla2=[]
#poser la question !!!!
print("A "+str(time[49])+", le sommet de la couche limite était plutôt à :")
print(pt_inflexion[0])
hcla_beg=input("Choisissez: ")
hcla_beg=int(hcla_beg)
hcla2.append(pt_inflexion[0][hcla_beg])
