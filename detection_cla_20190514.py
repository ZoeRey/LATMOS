...
plt.plot(data532paral[48][:200],altitude[:200])
plt.grid()
plt.show()
...
"""
time_hcla=time[48:90]
data532_medianfilter=[[] for i in range(len(time_hcla))]
xbis=[[] for i in range(len(time_hcla))]

for i in range(len(iymax)):
    if i<=5:
        data532_medianfilter[i]=data532_medianfilter[i]+list(data532paral[i+48][:42])
        xbis[i]=xbis[i]+list(altitude[:42])
    else:
        data532_medianfilter[i]=data532_medianfilter[i]+list(scipy.signal.medfilt(data532paral[i+48][iymax[i]-25:iymax[i]+70],3))
        xbis[i]=xbis[i]+list(altitude[iymax[i]-25:iymax[i]+70])
...
#poser la question !!!!
"""
print("A "+str(time[48])+", le sommet de la couche limite était plutôt à :")
print(pt_inflexion[0])
hcla_8h=input("Choisissez: ")
hcla_8h=int(hcla_8h)
hcla2.append(pt_inflexion[0][hcla_8h])
"""
hcla2.append(pt_inflexion[0][1])
